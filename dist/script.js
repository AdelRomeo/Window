...
var forms = function forms(state) {
  ...
  var inputList = document.querySelectorAll('input'); //список сообщений показываемых пользователю
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            document.querySelector('.status').textContent = messages.loading; //отправляем данные
...
var modals = function modals() {
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      //показываем модальное окно
      document.querySelector(selector).style.display = 'block'; //запрещаем скролл страницы на время показа модального окна
...
var tabs = function tabs(headerSelector, tabsSelector, contentsSelector, activeClass) {
  ...
  var tabs = document.querySelectorAll(tabsSelector); //элементы контента

  var contents = document.querySelectorAll(contentsSelector); //скрытие всех табов
  ...
  hideTabsContent();
  showTabContent(); //вешаем обработчик на контайнер табов

  header.addEventListener('click', function (e) {
    var target = e.target; //проверяем наличие класса у элемента на который кликнули
    ...
    if (target.classList.contains(tabsSelector.replace(/\./, '')) || target.parentNode.classList.contains(tabsSelector.replace(/\./, ''))) {
      ...
      tabs.forEach(function (tab, i) {
        ...
        if (target === tab || target.parentNode === tab) {
          //скрываем все
          hideTabsContent(); //показываем элементы с идексом таба на который кликнули
